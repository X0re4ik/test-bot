...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Давай знакомиться! Напиши свое имя", reply_markup=ReplyKeyboardRemove()
    )
    return ASK_NAME
...
async def ask_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Напиши свой табельный номер, чтобы я мог найти тебя в системе"
    )
...
async def ask_tab_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not tab.isdigit():
        await update.message.reply_text(
            "Табельный номер должен состоять только из цифр. Попробуй снова:"
        )
        return ASK_TAB_NUMBER
...
async def ask_first_day(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            "Неверный формат даты. Укажи в формате ДД.ММ.ГГГГ:"
        )
        return ASK_FIRST_DAY
    ...
    if text == "1. Сбер на Урале":
        ...
        if PRESENTATION_PATH.exists():
            try:
                ...
                await update.message.reply_text(
                    f"⚠️ Ошибка: {str(e)}", reply_markup=get_back_and_ask()
                )
        else:
            await update.message.reply_text(
                "⚠️ Презентация не найдена.", reply_markup=get_back_and_ask()
            )
        ...
        if PHOTO_P2P_PATH.exists():
            with open(PHOTO_P2P_PATH, "rb") as f:
                await update.message.reply_photo(
                    photo=InputFile(f), reply_markup=get_back_and_ask()
                )
        else:
            await update.message.reply_text(
                "🖼️ Схема временно недоступна.", reply_markup=get_back_and_ask()
            )
        ...
        await update.message.reply_text(
            "1. Опиши, что понравилось при использовании бота:"
        )
...
async def feedback_liked(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "2. Напиши, чего тебе не хватило при использовании бота:"
    )
...
async def feedback_missing(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "3. Что можно добавить в чат-бот, чтобы его использование было максимально полезным для новых сотрудников?"
    )
...
async def feedback_suggest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    update_feedback_in_excel(
        user.id,
        context.user_data.get("feedback_liked", ""),
        context.user_data.get("feedback_missing", ""),
        suggest,
    )
    await update.message.reply_text(
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "back_to_menu":
        await query.message.reply_text(
            "Выбери пункт меню:", reply_markup=get_main_menu()
        )
    elif query.data == "ask_question":
        await query.message.reply_text(
            "Напиши свой вопрос — мы обязательно ответим!",
            reply_markup=ReplyKeyboardMarkup(
                [["⬅️ Вернуться в меню"]], resize_keyboard=True
            ),
        )
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            ASK_TAB_NUMBER: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, ask_tab_number)
            ],
            ASK_FIRST_DAY: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, ask_first_day)
            ],
            MAIN_MENU: [
            ...
            FEEDBACK_LIKED: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, feedback_liked)
            ],
            FEEDBACK_MISSING: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, feedback_missing)
            ],
            FEEDBACK_SUGGEST: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, feedback_suggest)
            ],
        },
